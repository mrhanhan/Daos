...
public class EntityTable implements ITable {
     ...
     * V 字段信息对象
     */
    private Map<String, com.mrhan.db.entitydaos.EntityField> tableFields = new HashMap<>();
    ...
    private void parseField(Field [] fs){
        for(Field f : fs){//遍历集合
            ...
            if(c!=null){//如果不为空代表这是一个需要映射的字段
                ...
                com.mrhan.db.entitydaos.EntityField ef = new com.mrhan.db.entitydaos.EntityField(f,c);
    ...
    public Set<IField> fields() {
        ...
        for(com.mrhan.db.entitydaos.EntityField ef : tableFields.values()){
            fields.add(ef);
        }
        return fields;
    }
